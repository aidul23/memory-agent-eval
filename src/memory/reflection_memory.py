"""ReflectionMemory: feedback-driven reflection notes.
...
Historical note
---------------
This class was previously called ``HindsightMemory`` (registered as
``hindsight`` in the experiment configs). It was renamed to
``ReflectionMemory`` (registry key ``reflection``) to free up the
``hindsight`` slot for Vectorize.io's Hindsight memory service, which is
exposed as ``VectorizeHindsightMemory``. A thin backwards-compatibility shim
remains in ``hindsight_memory.py`` so old configs/code continue to work.
"""
...
class ReflectionMemory(BaseMemory):
    """In-RAM reflection store with Jaccard retrieval.

    This is our hand-rolled "hindsight reflection" baseline. It deliberately
    avoids embeddings so the comparison with real memory services
    (Mem0/Zep/AContext/Vectorize Hindsight) shows the lift those services
    provide over a simple bag-of-words approach.
    """
    ...
    def _build_reflection(interaction: dict[str, Any]) -> str:
        ...
        Format: "On scenario X (rule pack Y, session N, task T): violated
        rules [...]. Next time: ...".
